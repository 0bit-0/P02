...
while True:
    _,frame=cap.read()
    count = {
             'a': len(os.listdir(directory+"/dislike")),
             'b': len(os.listdir(directory+"/hello")),
             'c': len(os.listdir(directory+"/hope")),
             'd': len(os.listdir(directory+"/like")),
             'e': len(os.listdir(directory+"/patola")),
             'f': len(os.listdir(directory+"/thanku")),
             'g': len(os.listdir(directory+"/victory")),
             'h': len(os.listdir(directory+"/yes"))
             }
    ...
    if interrupt & 0xFF == ord('a'):
        cv2.imwrite(directory+'dislike/'+str(count['a'])+'.png',frame)
    if interrupt & 0xFF == ord('b'):
        cv2.imwrite(directory+'hello/'+str(count['b'])+'.png',frame)
    if interrupt & 0xFF == ord('c'):
        cv2.imwrite(directory+'hope/'+str(count['c'])+'.png',frame)
    if interrupt & 0xFF == ord('d'):
        cv2.imwrite(directory+'like/'+str(count['d'])+'.png',frame)
    if interrupt & 0xFF == ord('e'):
        cv2.imwrite(directory+'patola/'+str(count['e'])+'.png',frame)
    if interrupt & 0xFF == ord('f'):
        cv2.imwrite(directory+'thanku/'+str(count['f'])+'.png',frame)
    if interrupt & 0xFF == ord('g'):
        cv2.imwrite(directory+'victory/'+str(count['g'])+'.png',frame)
    if interrupt & 0xFF == ord('h'):
        cv2.imwrite(directory+'yes/'+str(count['i'])+'.png',frame)        
